...
public class BuyGetTest {
    ...
    @DisplayName("주어진 수량에 대해 무료로 제공할 수량을 계산하여 반환한다.")
    @ParameterizedTest
    @CsvSource(value = {"5:2:1:2", "7:3:1:2", "2:1:1:1", "3:1:1:2"}, delimiter = ':')
    void calculateFreeQuantity(final int quantity, final int buy, final int get, final int expected){
        BuyGet buyGet = BuyGet.of(buy, get);

        int actual = buyGet.calculateFreeQuantity(quantity);

        assertThat(actual).isEqualTo(expected);
    }

    @DisplayName("주어진 수량에 대해 필요한 프로모션 수량을 계산하여 반환한다.")
    @ParameterizedTest
    ...
    void calculateQuantityCondition(final int quantity, final int buy, final int get, final int expected){
        ...
        int actual = buyGet.calculateQuantityCondition(quantity);

        assertThat(actual).isEqualTo(expected);
    }

    @DisplayName("주어진 수량에 대해 프로모션을 적용 가능한 수량을 계산하여 반환한다.")
    @ParameterizedTest
    ...
    @DisplayName("주어진 수량에 대해 적용 가능한 무료 수량을 계산하여 반환한다.")
    @ParameterizedTest
